...
attachTransformScopes(ToolsResource, (scopes, parent) => {
  ...
  if (!scopes.modelContext && parent.modelContext.source === null) {
    return { ...scopes, modelContext: ModelContextResource() };
  }
  return scopes;
});
```
...
- `scopes` — the current scopes config being built
- `parent` — the parent `AssistantClient` (from the `AuiProvider` above)

It returns a new scopes config, optionally adding scopes that should be created alongside.
...
attachTransformScopes(MyResource, (scopes, parent) => {
  ...
  if (!scopes.tools && parent.tools.source === null) {
    return { ...scopes, tools: ToolsResource() };
  }
  return scopes;
});
```
