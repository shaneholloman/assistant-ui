...
<Callout type="warn">
Context is not properly persisted across `tapSubscribableResource` boundaries. If a child resource is wrapped with `tapSubscribableResource`, it may not see context values provided by its ancestors. This is a known bug.
</Callout>
