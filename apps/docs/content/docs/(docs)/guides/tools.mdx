...
## Tool UI vs Component Parts

Use **Tool UI** when the model is calling a tool (`type: "tool-call"`), you
need tool execution state (`running`, `requires-action`, `result`), or you want
argument/result inspection.

Use **component message parts** (`type: "component"`) when the server/runtime is
sending a presentational UI block directly.
...
Component part renderers receive the full part state (`name`, `props`,
`parentId`, `status`) and should treat `props` as JSON-serializable payload.
