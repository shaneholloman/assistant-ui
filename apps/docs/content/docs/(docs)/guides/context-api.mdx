...
aui.thread().getState();
aui.thread().getMessageByIndex(idx);
aui.thread().getMessageById(messageId);
aui.thread().composer();
...
aui.message().getMessagePartByIndex(idx);
aui.message().getMessagePartByToolCallId(toolCallId);
...
const messageById = aui.thread().getMessageById("msg_123");
const messageByIndex = aui.thread().getMessageByIndex(0);

// Access part by index or tool call ID
const partByIndex = aui.message().getMessagePartByIndex(0);
const partByToolCall = aui.message().getMessagePartByToolCallId("call_123");

// Access attachment by index
const attachment = aui.composer().getAttachmentByIndex(0).getState();

// Access thread list item by ID or index
const threadItem = aui.threads().getItemById("thread_123");
const threadByIndex = aui.threads().getItemByIndex(0);
```
...
function CopyButton() {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(aui.message().unstable_getCopyText());
  };
...
| Thread         | `append(message)`, `startRun()`, `resumeRun()`, `cancelRun()`             | Message handling and conversation control |
...
| Part           | `addToolResult(result)`, `getState()`                                     | Tool call result handling                 |
