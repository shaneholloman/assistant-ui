...
<Callout type="info">
  Native `component` parts are first-class assistant message parts with shape
  `{ type: "component", name, props?, parentId? }`. They render through
  `MessagePrimitive.Parts` and `MessagePrimitive.Unstable_PartsGrouped` via
  `components.Component` (`by_name`, `Fallback`, `Override`).
  Legacy `aui/v0` serialization does not support `component` parts.
</Callout>
      ...
      children: [
        {
          ...
          parameters: [
            ...
            },
            {
              ...
              type: "object",
              description:
                "Configuration for native component message parts (`type: \"component\"`).",
              children: [
                {
                  parameters: [
                    {
                      name: "by_name",
                      type: "Record<string, ComponentMessagePartComponent>",
                      description:
                        "A map of component names to renderer components.",
                    },
                    {
                      name: "Fallback",
                      type: "ComponentMessagePartComponent",
                      description:
                        "Fallback renderer used when no `by_name` entry exists.",
                    },
                    {
                      name: "Override",
                      type: "ComponentMessagePartComponent",
                      description:
                        "Single renderer that handles all component message parts.",
                    },
                  ],
                },
              ],
...
<MessagePrimitive.Parts
  components={{
    Component: {
      by_name: {
        "status-chip": ({ props }) => (
          <StatusChip label={String(props?.label ?? "")} />
        ),
      },
      Fallback: ({ name }) => <div>Unknown component: {name}</div>,
    },
  }}
/>
```
