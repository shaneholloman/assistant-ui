...
```tsx
<ModelSelector variant="ghost" />   // No background (default)
<ModelSelector variant="outline" /> // Border only
<ModelSelector variant="default" /> // Solid background
```

| Variant   | Description                                    |
| --------- | ---------------------------------------------- |
| `ghost`   | No background, subtle hover (default)          |
| `outline` | Border with transparent background             |
| `default` | Solid secondary background                     |
...
```tsx
<ModelSelector size="sm" />      // Compact (h-7, text-xs)
<ModelSelector size="default" /> // Standard (h-8)
<ModelSelector size="lg" />      // Large (h-9)
```
...
<ModelSelectorRoot models={models} value={modelId} onValueChange={setModelId}>
  <ModelSelectorTrigger variant="outline" size="lg" />
  <ModelSelectorContent />
</ModelSelectorRoot>
```
