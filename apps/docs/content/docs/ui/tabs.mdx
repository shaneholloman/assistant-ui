...
Use the `variant` prop on `TabsList` to change the visual style. The variant is automatically inherited by child `TabsTrigger` components.
...
Use the `size` prop on `TabsList` to change the tab height.
...
| `TabsList` | The container for tab triggers. Accepts `variant` and `size` props. |
| `TabsTrigger` | An individual tab button. Inherits `variant` and `size` from `TabsList`. |
  ...
  parameters={[
    {
      ...
      default: '"default"',
      description: "The visual style of the tabs. Inherited by child TabsTrigger components.",
    },
    {
      name: "size",
      type: '"sm" | "default" | "lg"',
      default: '"default"',
      description: "The size of the tabs. Inherited by child TabsTrigger components.",
    },
    {
      name: "className",
      type: "string",
...
<ParametersTable
  ...
  parameters={[
    ...
    },
    {
      name: "variant",
      type: '"default" | "line" | "ghost" | "pills" | "outline"',
      description: "Override the variant from TabsList.",
    },
    {
      name: "size",
      type: '"sm" | "default" | "lg"',
      description: "Override the size from TabsList.",
    },
    {
      name: "asChild",
      type: "boolean",
...
| `tabsTriggerVariants` | Styles for individual tab triggers. |
...
<div className={tabsListVariants({ variant: "ghost", size: "sm" })}>
  <button className={tabsTriggerVariants({ variant: "ghost", size: "sm" })}>
    Custom Tab
  </button>
</div>
```
