...
<Callout type="info">
If you are already using `@langchain/react`'s `useStream` hook, the alternative [`@assistant-ui/react-langchain`](/docs/runtimes/langchain) adapter may fit better. It is a thinner wrapper but currently exposes fewer features. See [LangChain useStream](/docs/runtimes/langchain) for a feature gap table.
</Callout>
