...
<Callout type="info">
This is an integration guide, not a runtime adapter. assistant-ui does not ship a `@assistant-ui/react-mastra` package. You wire up Mastra through the standard [Vercel AI SDK runtime](/docs/runtimes/ai-sdk/v6) by routing your API endpoint through Mastra's agent stream. The guides below show the two common shapes.
</Callout>
...
Mastra integrates at the LLM-client layer (server-side). assistant-ui talks to your API route via the AI SDK runtime; your API route asks Mastra to run an agent and returns the stream. This is layered on `ExternalStoreRuntime` (see [architecture](/docs/runtimes/concepts/architecture)) through the AI SDK adapter.
