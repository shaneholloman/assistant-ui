...
| Prop | Type | Description |
|------|------|-------------|
| `components` | `{ Suggestion: FC }` | Component used to render each suggestion |
...
<MessagePrimitive.Parts
  components={{
    ...
    tools: {
      Fallback: ({ part }) => <Text dimColor>[Tool: {part.toolName}]</Text>,
    },
  }}
/>
```
...
<ChainOfThoughtPrimitive.Root>
  ...
  <ChainOfThoughtPrimitive.Parts
    components={{
      Text: ({ text }) => <Text dimColor>{text}</Text>,
    }}
  />
</ChainOfThoughtPrimitive.Root>
```
...
<MessagePrimitive.Parts
  components={{
    tools: {
      Fallback: ({ part }) => <ToolCallPrimitive.Fallback part={part} />,
    },
  }}
/>
```
